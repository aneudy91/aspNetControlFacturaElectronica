...
GO
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[TblClienteEmpresa]') AND type in (N'U'))
    DROP TABLE [dbo].[TblClienteEmpresa]
GO
CREATE TABLE TblClienteEmpresa(
	 IDClienteEmpresa int identity(1,1) primary key not null
	,IDEmpresa int CONSTRAINT fk_TblClienteEmpresa_IDEmpresa FOREIGN KEY REFERENCES TblEmpresa(IDEmpresa) on delete cascade 
	,NombreComercial varchar(300)
	...
	,TipoPersona varchar(20) /*Fisica o Moral*/
...
CREATE TABLE TblUsuarios(
	...
	,IDEmpresa int CONSTRAINT fk_TblUsuarios_IDEmpresa FOREIGN KEY REFERENCES TblEmpresa(IDEmpresa)  
	,IDClienteEmpresa int CONSTRAINT fk_TblUsuarios_IDClienteEmpresa FOREIGN KEY REFERENCES TblClienteEmpresa(IDClienteEmpresa) on delete cascade 
