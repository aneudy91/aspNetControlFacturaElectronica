...
Declare @msj nvarchar(255),@UsuarioRFC varchar(20)

select @UsuarioRFC=RFC from TblClienteEmpresa with (nolock) where IDClienteEmpresa = @IDClienteEmpresa;

BEGIN     
if not exists (SELECT * 
				FROM TblFacturas with (nolock)
				WHERE Comprobante_folio =@Comprobante_folio and @UsuarioRFC in (Emisor_RFC,Receptor_RFC)) 
begin
...
begin
	set @msj='El Folio <b>'+@Comprobante_folio+'</b> ya existe para el RFC <b>'+@UsuarioRFC+'</b>';
	RAISERROR (@msj, 15,1)
	RETURN -100;
end;    
END
