...
HISTORIAL DE CAMBIOS | FECHA      RESPONSABLE       MOTIVO
                     | ---------- ---------------------------------------
                     | 09/09/2015 Aneudy Abreu   - Se agregó un validación para el IDClienteEmpresa = 1 (Prueba)
                     |                             para que cuando se suban facturas se le agregen los 
                     |                             dígitos AAC a su folio para no interferir con las facturas 
                     |                             subidas por el cliente en producción.
                     | VERSIÓN 2.0                               
                     |                               
-------------------------------------------------------------------------
-- =================================================================================================================  
...
AS     
  ...
  if (@IDClienteEmpresa = 1)
  begin
	set @Comprobante_folio = @Comprobante_folio + 'AAC';
  end
  
  
 Declare @msj nvarchar(255)      
BEGIN       
if not exists (SELECT * FROM TblFacturas WHERE Receptor_RFC = @Receptor_RFC and Comprobante_folio =@Comprobante_folio)   
begin  
...
begin  
 set @msj='El Folio <b>'+@Comprobante_folio+'</b> ya existe para el RFC <b>'+@Receptor_RFC+'</b>';  
 RAISERROR (@msj, 15,1)  
 RETURN -100;  
end;      
END  